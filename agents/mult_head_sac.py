...
class MultiHeadSAC(DeepAC):
    ...
    def __init__(
        ...
        if self._buffer_strategy == "uniform":
            ...
            self._replay_memory = PrioritizedReplayMemory(
                initial_replay_size,
                max_replay_size,
                alpha=self._buffer_alpha,
                beta=self._buffer_beta,
            )
    ...
    def fit(self, dataset):
        ...
        if self._replay_memory.initialized:
            ...
            q = (
                self._reward_scale
                * np.repeat(np.expand_dims(reward, axis=1), q_next.shape[-1], axis=1)
    ...
    def _next_q(self, next_state, absorbing):
        ...
        log_prob_next = np.repeat(
            np.expand_dims(log_prob_next, axis=1), q.shape[-1], axis=1
        )
        q = q - self._alpha_np * log_prob_next
        q *= np.ones(q.shape) - np.repeat(
            np.expand_dims(absorbing, axis=1), q.shape[-1], axis=1
        )
